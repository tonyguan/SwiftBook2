...
class GamePlayScene: SKScene, SKPhysicsContactDelegate {
    ...
    func initBG() {
        ...
        sprite1.name = "bgSprite"
        addChild(sprite1)

        let ac1 = SKAction.move(by: CGVector(dx: 500, dy: 600), duration: 20)
        let ac2 = ac1.reversed()
        let as1 = SKAction.sequence([ac1, ac2])
        sprite1.run(SKAction.repeatForever(as1))

        //添加背景精灵2.
        let sprite2 = SKSpriteNode(texture: gameplayAtlas.textureNamed("bgSprite2"))
        sprite2.position = CGPoint(x: self.frame.width, y: 0)
        sprite2.name = "bgSprite"
        addChild(sprite2)

        let ac3 = SKAction.move(by: CGVector(dx: -500, dy: 600), duration: 10)
        let ac4 = ac3.reversed()
        let as2 = SKAction.sequence([ac3, ac4])
        sprite2.run(SKAction.repeatForever(as2))
    ...
    func initFighter() {
        ...
        addChild(fighter!)
        fighter?.position = CGPoint(x: self.frame.midX, y: 70)
    ...
    func initEnemys() {
        ...
        //设置敌人物体碰撞检测
        self.enumerateChildNodes(withName: "enemy") { (node: SKNode!,
    ...
    func initScoreBar() {
        ...
        //获得生命值标签对象
        labelLife = childNode(withName: "labelLife") as? SKLabelNode
        ...
        gameplayAtlas.textureNamed("buttonPause"),
                ...
                callback: #selector(touchedPauseButton))
    ...
    override func touchesMoved(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        for touch in touches {
            ...
            if xPos < halfWidth {
                //不能超过左边屏幕
                xPos = halfWidth
            } else if xPos > (sceneWidth - halfWidth) {
                //不能超过右边屏幕
            ...
            if yPos < halfHeight {
                //不能超过底边屏幕
                yPos = halfHeight
            } else if yPos > (sceneHeight - halfHeight) {
                //不能超过上边屏幕
    ...
    override func update(_ currentTime: TimeInterval) {
        ...
        for node in self.children where node is Bullet && self.pauseStatus == false {
            ...
            bullet.update(currentTime: currentTime)
        }

        //计算子弹发射时间
        let timeSinceLastUpdate = currentTime - lastUpdateTime
        if timeSinceLastUpdate >= timeDelta && self.pauseStatus == false {
            //发射子弹
            self.shootBullet()
    ...
    func touchedPauseButton() {
        ...
        self.enumerateChildNodes(withName: "enemy") { (node: SKNode!,
                                                       stop: UnsafeMutablePointer<ObjCBool>) -> Void in
            node.isPaused = self.pauseStatus
        }
        //设置背景精灵暂停
        self.enumerateChildNodes(withName: "bgSprite") { (node: SKNode!,
                                                          stop: UnsafeMutablePointer<ObjCBool>) -> Void in
            node.isPaused = self.pauseStatus
        }

        //设置我的飞机暂停
        fighter?.isPaused = pauseStatus
        ...
        gameplayAtlas.textureNamed("buttonBack"),
                ...
                callback: #selector(touchedBackMenuItem))
        backMenuItem.zPosition = 30
        backMenuItem.position = CGPoint(x: self.frame.midX, y: self.frame.midY + 50)
        ...
        gameplayAtlas.textureNamed("buttonResume"),
                ...
                callback: #selector(touchedResumeMenuItem))
        resumeMenuItem.zPosition = 30
        resumeMenuItem.position = CGPoint(x: self.frame.midX, y: self.frame.midY - 20)
    ...
    func touchedResumeMenuItem() {
        ...
        //移除菜单
        self.enumerateChildNodes(withName: "menu") { (node: SKNode!,
                                                      stop: UnsafeMutablePointer<ObjCBool>) -> Void in
    ...
    func shootBullet() {
        ...
        if fighter != nil && fighter?.isHidden == false {
            ...
            bullet.shootBulletFromFighter(fighter: fighter!)
        }
    }
    ...
    func didBegin(_ contact: SKPhysicsContact) {
        ...
        //过滤掉隐藏的节点
        if nodeB.isHidden || nodeA.isHidden {
            return
        }
    ...
    //MARK: --子弹与敌人的碰撞检测
    func handleBulletCollidingWithEnemy(_ enemy: Enemy) {
    ...
    //MARK: --处理玩家与敌人的碰撞检测
    func handleFighterCollidingWithEnemy(_ enemy: Enemy) {
        ...
        if fighter!.hitPoints > 0 {
            ...
            let ac1 = SKAction.move(to: CGPoint(x: self.size.width / 2, y: 70), duration: 0.0)
            let ac2 = SKAction.fadeOut(withDuration: 0.1)
            let acgruop1 = SKAction.group([ac1, ac2])

            //显示动作相当于Hide设置为false
            let ac3 = SKAction.unhide()
            let ac4 = SKAction.fadeIn(withDuration: 1.0)
