...
class Enemy: SKSpriteNode {
    ...
    init(enemyType: EnemyTypes, velocity: CGVector) {
        ...
        //其他设置
        self.isHidden = true //设置当前精灵隐藏

        switch enemyType {
        case .EnemyTypeStone:
            //一秒钟旋转180度
            let ac = SKAction.rotate(byAngle: CGFloat(M_PI), duration: 0.5)
            self.run(SKAction.repeatForever(ac))
            //设置敌人物理引擎的物体
        ...
        case .EnemyTypePlanet:
            //一秒钟旋转-180度
            let ac = SKAction.rotate(byAngle: CGFloat(-1 * M_PI), duration: 2)
            self.run(SKAction.repeatForever(ac))
            //设置敌人物理引擎的物体
            let enemyBody = SKPhysicsBody(circleOfRadius: self.frame.width / 2 - 5)
            self.physicsBody = enemyBody
        ...
        case .EnemyTypeEnemy1:
            ...
            path.move(to: CGPoint(x: -2.5, y: -45.75))
            path.addLine(to: CGPoint(x: -29.5, y: -27.25))
            path.addLine(to: CGPoint(x: -53, y: -0.25))
            path.addLine(to: CGPoint(x: -34, y: 43.25))
            path.addLine(to: CGPoint(x: 28, y: 44.25))
            path.addLine(to: CGPoint(x: 55, y: -2.25))

            let enemyBody = SKPhysicsBody(polygonFrom: path)
            self.physicsBody = enemyBody
    ...
    func update(currentTime: CFTimeInterval) {
        ...
        //每次渲染一帧调用一次。
        let ac = SKAction.move(by: velocity, duration: 0.0)
        self.run(ac)
    }
    ...
    func spawn() {
        ...
        let ac = SKAction.move(to: CGPoint(x: xPos, y: yPos), duration: 0.0)
        self.run(ac)
