...
class Employee: NSObject {
    ...
    init(no: Int, name: String, job: String, salary: Double, hiredate: Date, dept: Department) {
        self.no = no
        self.name = name
        self.job = job
        //工资
...
//打印Employee集合
func printEmployees(_ employees: NSArray) {
    for item in employees {
        let emp = item as! Employee
        print("no: \(emp.no) name:  \(emp.name) salary: \(emp.salary)  job:  \(emp.job)")
    }
}
