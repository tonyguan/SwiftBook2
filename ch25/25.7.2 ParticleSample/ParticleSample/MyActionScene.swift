...
class MyActionScene: SKScene {
    ...
    override func didMove(to view: SKView) {
        ...
        let backLabel = WKLabelButton(text: "<Back", fontNamed:"Chalkduster", callback: #selector(touchBackLabel))
        ...
        label.position = CGPoint(x:self.frame.midX, y:self.frame.height - 60)
        self.addChild(label)
    ...
    func touchBackLabel() {
        let push = SKTransition.push(with: .left, duration: 0.6)
